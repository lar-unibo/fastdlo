...
class PredictData():
    ...
    @ staticmethod
    def buildNodeFeaturesTopDown(nodes_dict, pos_int, pos_color, node_id):

        attr_id = nodes_dict[node_id]
               
        pos_nn = attr_id["pos_norm"]
        dir = np.array(pos_int) - np.array(pos_nn)
        ...
        vector = [  float(pos_d[0]), float(pos_d[1]), float(angle),
                    float(attr_id["radius"]),
                    float(pos_color[0]), float(pos_color[1]), float(pos_color[2])]
        return vector
    ...
    def getAllPairsFromMemory(path):
        ...
        return PredictData.makeTensors(nodes, pred_edges)
    ...
    def makeTensors(nodes, pred_edges):
        ...
        for e in pred_edges:
            ...
            x0 = PredictData.buildNodeFeatures(nodes, node_id = e[0])
            x0 = torch.Tensor(x0)

            x1 = PredictData.buildNodeFeatures(nodes, node_id = e[1])
            x1 = torch.Tensor(x1)
    ...
    @ staticmethod
    def makeTensorsTopDown(data_list, nodes):
        rv = []
        for v in data_list:
            #print(v)

            for vv in [v["c0"][0], v["c0"][1]]:

                x0 = PredictData.buildNodeFeatures(nodes, node_id = vv)
                x0 = torch.Tensor(x0)            

                x0int = PredictData.buildNodeFeaturesTopDown(nodes, pos_int=v["pos_int"], pos_color=v["color_int"], node_id = vv)
                x0int = torch.Tensor(x0int)
                rv.append({ "node_0": vv, "node_1": -1, "feat_0": x0, "feat_1": x0int})

            for vv in [v["c1"][0], v["c1"][1]]:

                x1 = PredictData.buildNodeFeatures(nodes, node_id = vv)
                x1 = torch.Tensor(x1)    

                x1int = PredictData.buildNodeFeaturesTopDown(nodes, pos_int=v["pos_int"], pos_color=v["color_int"], node_id = vv)
                x1int = torch.Tensor(x1int)
                rv.append({ "node_0": vv, "node_1": -1, "feat_0": x1, "feat_1": x1int})
